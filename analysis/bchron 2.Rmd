...
```{r Installing Bchron}
getwd()
setwd(dir = "Desktop/Thesis/Data/bchron")
...
red_c14 <- read.csv(file = "red14_bchron.csv")
...
So far, the one downside is that I'll need to enter each of my dates in a vector. Since I have 45, I'm going to try a work around. 

```{r eval=FALSE, include=FALSE}
red_ages <- red_c14$ages
red_SDs <- red_c14$SD
red_depth <- red_c14$depth
red_cal <- rep("intcal13", times = length(red_ages))
all_ages = BchronCalibrate(ages = red_ages,
                           ageSds = red_SDs,
                           positions = red_depth,
                           calCurves = red_cal)
summary(all_ages)
...
```{r BchronDensity}
# red_density = BchronDensity(ages = red_ages, ageSds = red_SDs, calCurves = red_cal)
# plot(red_density)
# rm(red_density)
```
...
getwd()
dates <- read.csv("dates.csv")
# should take "modern" out of age for now, turning ages into factors when they need to be numeric
  # ended up doing in excel

# plotting
# ggplot(dates, aes(x = dates$median_cal_age, y = dates$depth, color = factor(dates$site))) + geom_point() + facet_grid(.~dates$site_type, scales = "fixed")  + background_grid(major="xy") + scale_y_reverse()  + panel_border()
```
...
```{r}
# parsing out mineral soil data
valley_ms <- dates[which(dates$site_type == "valley mineral soil"),]
hillslope_ms <- dates[which(dates$site_type == "hillslope mineral soil"),]
ridgetop_ms <- dates[which(dates$site_type == "ridgetop mineral soil"),]
ms_dates <- merge(valley_ms, ridgetop_ms, all = TRUE)
ms_dates <- merge(ms_dates, hillslope_ms, all = TRUE)

ms_ggplot <- ggplot(ms_dates, aes(x = ms_dates$median_cal_age, y = ms_dates$depth, color = ms_dates$site_type, fill = ms_dates$site_type, shape = ms_dates$site_type)) + 
  geom_point(size = 4) + 
  ...
  scale_fill_manual(values = c("#20202080", "#E69F0080", "#56B4E980"),
                    labels = c("Hillslope Mineral Soil", "Ridgetop Mineral Soil", "Valley Mineral Soil"),
                    name = "Site Type:") + 
  background_grid() + panel_border() + scale_y_reverse() + labs(x = "Age (cal year BP)", y = "Depth (cm)", title = "Forest Mineral Soil Sites")+ 
  theme(legend.position = "right")
ms_ggplot
save_plot("ms_ggplot.png", ms_ggplot, base_height = 4, base_width = NULL, base_aspect_ratio = 1.5)
```

That's a start. Let's do the same with the other data types:
```{r}
alluvial_dates <- dates[which(dates$site_type == "alluvial fan"),]
colluvial_dates <- dates[which(dates$site_type == "colluvial hollow"),]
ns_dates <- merge(alluvial_dates, colluvial_dates, all = TRUE)

ns_ggplot <- ggplot(ns_dates, aes(x = ns_dates$median_cal_age, y = ns_dates$depth, color = ns_dates$site_type, fill = ns_dates$site_type, shape = ns_dates$site_type)) + 
  geom_point(size = 4)+ 
  ...
  scale_color_manual(values = c("#D55E00", "#009E73"),
                     name = "Site Type:",
                     labels = c("Alluvial Fan", "Colluvial Hollow")) +
  scale_fill_manual(values = c("#D55E0080", "#009E7380"),
                    name = "Site Type:",
                    labels = c("Alluvial Fan", "Colluvial Hollow")) + 
  background_grid() + scale_y_reverse() + labs(x = "Age (cal year BP)", y = "Depth (cm)", title = "Depositional Soil Sites") + panel_border() +
  theme(legend.position = "right")
ns_ggplot
save_plot("ns_ggplot.png", ns_ggplot, base_height = 4, base_width = NULL, base_aspect_ratio = 1.5)
```
