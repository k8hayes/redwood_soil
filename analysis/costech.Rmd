...
dataloc = "C:/Users/khayes23/Documents/Research/R20 Repositories/R200 Active/redwood_soil/data/"
costech_results <- read.csv(paste0(dataloc, "costech_results.csv"))
costech_results
...
```{r}
 site_ggplot <- ggplot(sites, aes(x = site_id, y = mgC_g, fill = type)) + 
  geom_boxplot() + 
...
depth_ggplot <- sites %>%
  ...
  ggplot(aes(x = depth_category, y = mgC_g, fill = type)) +
  geom_boxplot() + background_grid() + panel_border() +
  labs(x = "Depth (cm)", y = "mg of C / g of soil", title = "Difference in C across depth") +
  scale_fill_discrete(name = "",
                      labels =  c("Pyrogenic Carbon", "Total Soil carbon")) +
  scale_x_discrete(labels = c("0-5", "5-10", "10-15", "15-20", "20-25", "25-30"))
depth_ggplot
save_plot("depth.png", depth_ggplot, base_aspect_ratio = 1.8)

# just the total carbon
total_c_ggplot <- raw_samples %>%
  filter(depth_category != "G") %>%
  ggplot(aes(x = depth_category, y = mgC_g)) +
  geom_boxplot(fill = "#1c9099") + background_grid() + panel_border() +
  labs(x = "Depth (cm)", y = "Milligrams of C per g of soil", title = "Difference in total C across depth") + scale_x_discrete(labels = c("0-5", "5-10", "10-15", "15-20", "20-25", "25-30"))
total_c_ggplot
...
Doing this in excel
```{r}
ratios <- read.csv(paste0(dataloc, "ratios.csv"))
names(ratios)
...
ratio_pyc_mean <- tapply(ratio.of.pyc_total, depth_category, mean)
mean(ratio_pyc_mean)
ratio_ggplot <- ratios %>%
  filter(depth_category != "G") %>%
   ggplot( aes(x = depth_category, y = ratio.of.pyc_total)) +
