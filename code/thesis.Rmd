...
names(char_mass) # note: check names(char_mass) to make sure code is carrying through to next chunk (5/5/18)
attach(char_mass)
...
char_mass$bulk_density <- (dry_weight/ring_volume) # if this isn't working, check if you've attached char_mass again

# exploring relationship between bulk density and depth
ggplot(char_mass) + geom_boxplot(aes(char_mass$sample_id, char_mass$bulk_density), fill = "grey") + labs(x = "Depth category", y = "Bulk Density (grams per cubic centimeter)", title = "Bulk Density across depth")
